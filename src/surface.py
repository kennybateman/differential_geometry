...
class Bowl:
    ...
    def make_quadratic_from_measured_r_z_points(points):
        points = np.asarray(points)
        r_points = points[:,3]
        z_points = points[:,2]
        print(r_points)
        ...
        a, r0, z0 = popt
        print(f"a:{a} r0:{r0} z0:{z0}")

        bowl = Bowl(a=a, b=None, r0=r0, z0=z0)
    ...
    def zOfRQuadratic(r, a, r0, z0):
        return a * (r - r0)**2 + z0


    def getZofXYQuadratic(self, x, y):
        r = np.sqrt(x**2 + y**2)
        return self.getZOfRQuatratic(r)

    def getZOfRQuatratic(self, r):
        return Bowl.zOfRQuadratic(r, a=self.a, r0=self.r0, z0=self.z0)


    def _sampleQuadratic(self, size=400, resolution=200):
        u = np.linspace(-size, size, resolution)
        v = np.linspace(-size, size, resolution)
        X, Y = np.meshgrid(u, v)
        Z = self.getZofXYQuadratic(X,Y)
        return np.array([X, Y, Z])

    
    def sampleQuadratic(self, size=1, resolution=1000):
        self.size = size
        self.resolution = resolution
        if self.points.size == 0:
            self.points = self._sampleQuadratic(size, resolution)
        return self.points



    def _mask_out_inner_radius(points, radius):
        X, Y, Z = points
        R = np.sqrt(X**2 + Y**2)
        mask = R >= radius
    ...
    def mask_out_inner_radius(self, radius):
        points = self.sampleQuadratic()
        self.points = Bowl._mask_out_inner_radius(points, radius)
        return self.points


    def _mask_out_inner_z(points, minz):
        X, Y, Z = points
        mask = Z >= minz
        Z = np.where(mask, Z, np.nan)
        return np.array([X, Y, Z])

    def mask_out_minimum_z(self, minz):
        points = self.sampleQuadratic()
        self.points = Bowl._mask_out_inner_z(points, minz)
        return self.points


    # several points (r, z)
    # def make_exponential_from_measured_r_z_points(points):
    #     points = np.asarray(points)
    #     r_points = points[:,3]
    #     z_points = points[:,2]
    #     print(r_points)
    ...
    #     a0 = z_points.max() - z_points.min()
    #     b0 = 1 / r_points.max()
    #     z00 = z_points.min()
    #     print(f"initial param guesses: a:{a0} b:{b0} c:{z00}")
    #     params, covariance = curve_fit(
    #         Bowl._zOfRExponential,
    #         r_points,
    #         z_points,
    #         p0=[a0, b0, z00], # starting params I guess
    #     )
    #     a, b, z0 = params
    #     print(f"fit params for bowl: a:{a} b:{b} z0:{z0}")
    ...
    points = np.array([])
    def __init__(self, a=10, b=0.05, r0=0, z0=0):
        self.a = a
        self.b = b
        self.r0 = r0
        self.z0 = z0
    ...
    # def getZofXYExponential(self, x, y):
    #     return Bowl._zOfXYExponential(self.a, self.b, x, y, self.z0)

    # def getZofRExponential(self, r):
    #     return Bowl._zOfRExponential(self.a, self.b, r, self.z0)


    # def _zOfXYExponential(a, b, x, y, z0):
    #     r = np.sqrt(x**2 + y**2)
    #     return Bowl._zOfRExponential(a, b, r, z0)

    # def _zOfRExponential(a, b, r, z0):
    #     return a * (1 - np.exp(-b * r)) + z0
    ...
    #     v = np.linspace(-size, size, resolution)
    #     X, Y = np.meshgrid(u, v)
    #     Z = self.getZofXYExponential(X,Y)
    #     return np.array([X, Y, Z])
...
#     def getZofR(self, r):
#         return Bowl._zOfRExponential(r, self.a, self.b, self.z0)

#     def _zOfRExponential(r, a, b, z0):
